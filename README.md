...
いるため（間欠的で、同じ手順でも再現しない）。放っておくと**正本から消えた doc が
エージェントの検索で返り続ける**ので、静かに間違える。

確認して消えていればそれで終わり（Ingestion は 1 回）。残っていたときだけ取り込み直して
もう一度確認し、それでも残れば異常終了する。**回数ではなく状態で判断する**ので、
「消えたはずだが確認していない」状態にはならない。
