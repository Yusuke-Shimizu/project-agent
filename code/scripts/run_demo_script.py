...
ABSENCE = re.compile(
    # **正本／記録に対する否定**であることを見る。「決めていない」「判断材料が無い」だけでは
    # 足りない（それは相手の状況の話で、正本を引いた結果ではない）。
    #
    # 語尾を狭く列挙して 2 回取りこぼした（`記録は無い` しか見ておらず、
    # 「決定は正本に無い」「記録は正本に見つからない」を FAIL にしていた）。
    # **何を否定しているか**で見るほうが安定する
    r"(正本|記録|decision|knowledge)[^。\n]{0,25}?"
    r"(見つか(らな|りませ)|見当たら(な|ませ)|存在し(ない|ませ)|無い|ありませ)"
)
